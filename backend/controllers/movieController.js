...
const newMovie = async (req,res)=>{
  try{
   const latestMovies = await Movie.find().sort({createdAt:-1}).limit(10)
...
const randomMovies = async (req,res)=>{
   try{
     const getRandomMovies = await Movie.aggregate([{$sample:{size:10}}])
     res.json(getRandomMovies)
   }catch(error){
     res.status(500).json({error:error.message})
   }
}
...
export {createMovie,getAllMovies,getSpecificMovie,updateMovie,movieReview,
  deleteMovie,deleteComment,newMovie,getTopMovie,randomMovies}